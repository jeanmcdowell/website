...
const HalfCircle = ({ size = 100, fill = "var(--blue)", rotation = 0, style = {}, className = "", ...rest }) => (
  <svg className={className} width={size} height={size} viewBox="0 0 100 100" style={{ ...style, transform: style.transform || `rotate(${rotation}deg)` }} aria-hidden="true" {...rest}>
    <path d="M 2 50 A 48 48 0 0 1 98 50 Z" fill={fill}/>
  </svg>
);

const Square = ({ size = 100, fill = "var(--yellow)", rotation = 0, style = {}, className = "", ...rest }) => (
  <svg className={className} width={size} height={size} viewBox="0 0 100 100" style={{ ...style, transform: style.transform || `rotate(${rotation}deg)` }} aria-hidden="true" {...rest}>
    <rect x="2" y="2" width="96" height="96" fill={fill}/>
  </svg>
);

const Triangle = ({ size = 100, fill = "var(--blue)", rotation = 0, style = {}, className = "", ...rest }) => (
  <svg className={className} width={size} height={size} viewBox="0 0 100 100" style={{ ...style, transform: style.transform || `rotate(${rotation}deg)` }} aria-hidden="true" {...rest}>
    <polygon points="50,4 96,96 4,96" fill={fill}/>
  </svg>
);

const QuarterCircle = ({ size = 100, fill = "var(--red)", rotation = 0, style = {}, className = "", ...rest }) => (
  <svg className={className} width={size} height={size} viewBox="0 0 100 100" style={{ ...style, transform: style.transform || `rotate(${rotation}deg)` }} aria-hidden="true" {...rest}>
...
const Target = ({ size = 100, color = "var(--ink)", style = {}, className = "", ...rest }) => (
  <svg className={className} width={size} height={size} viewBox="0 0 100 100" style={style} aria-hidden="true" {...rest}>
